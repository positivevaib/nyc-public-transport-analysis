...
public class CleanMapper extends Mapper<LongWritable, Text, Text, Text> {
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        if (!isDateValid(line[3])) {
            return;
        }

        int year = Integer.parseInt(line[3].split("-")[2]); 
        int month = Integer.parseInt(line[3].split("-")[0]);
        int day = Integer.parseInt(line[3].split("-")[1]);

        int hour = Integer.parseInt(line[4].split(":")[0]);
        if (hour < 0 || hour > 23) {
            return; 
        }

        String description = line[5].trim();
        if (description.matches("REGULAR") == false) {
            return;           
        }

        int entries = Integer.parseInt(line[6].trim());
        if (entries < 0){
            return; 
        }   

        int exits = Integer.parseInt(line[7].trim());
        if (exits < 0){
            return; 
        }   
